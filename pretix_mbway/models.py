...
class MBWAYGatewayObject(models.Model):
    ...
    payment = models.ForeignKey('pretixbase.OrderPayment', null=True, blank=True,
                                on_delete=models.CASCADE)  # TODO -> we might need it to be unique os we can search by payment (on matching id for example) | -?-> is on cascade from deleted payment or if this is deleted it deletes the payment??
